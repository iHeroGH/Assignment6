...
screen_ob = Screen()
screen = screen_ob.screen
clock = screen_ob.clock
...
def draw_fence():
    ...
    for x in range(5, 800, 30):
        pygame.draw.polygon(screen, Color.NIGHT_GRAY, [[x + 2, y], [x + 2, y + 15], [x, y + 15], [x, y]])
...
while not done:
    ...
    screen_ob.move_clouds()
            
    # Drawing code (Describe the picture. It isn't actually drawn yet.)
    screen.fill(sky_color)
    screen_ob.SEE_THROUGH.fill(Color.COLOR_KEY)
    screen_ob.SEE_THROUGH.set_colorkey(Color.COLOR_KEY)
    
    if not day:
        screen_ob.draw_stars()
        

    ''' sun and moon '''
    draw_sun_or_moon()

    screen_ob.draw_clouds(cloud_color)

    screen.blit(screen_ob.SEE_THROUGH, (0, 0))   
    ...
    def draw_field():
        ...
        pygame.draw.arc(screen, Color.WHITE, [330, 280, 140, 40], math.pi, 2 * math.pi, 5)
        ...
        #stands right
        pygame.draw.polygon(screen, Color.RED, [[680, 220], [800, 340], [800, 290], [680, 180]])
        pygame.draw.polygon(screen, Color.WHITE, [[680, 180], [800, 100], [800, 290]])

    
        #stands left
        pygame.draw.polygon(screen, Color.RED, [[120, 220], [0, 340], [0, 290], [120, 180]])
    ...
    if not day and not lights_on:
        screen.blit(screen_ob.DARKNESS, (0, 0))    
    ...
    clock.tick(screen_ob.refresh_rate)
